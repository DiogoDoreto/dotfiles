# Zig package dependencies for the ghostty commit vendored by ghostel.
# Ghostty submodule: ghostty-org/ghostty @ c2dd7579e28ff1fecb4a68f32ae8cacda576550c
# (version 1.3.2-dev, minimum_zig_version = "0.15.2")
#
# Based on nixpkgs pkgs/by-name/gh/ghostty/deps.nix (ghostty v1.3.1).
# Two entries differ from v1.3.1 and are updated below:
#   - wayland_protocols: switched URL source (gitlab vs deps.files.ghostty.org)
#   - iterm2_themes: newer release tarball
# C library deps (highway, simdutf, etc.) are local path deps inside ghostty source
# (pkg/*) and are compiled from source 2014 no pre-fetching needed.
{
  lib,
  linkFarm,
  fetchurl,
